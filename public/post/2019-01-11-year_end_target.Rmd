...
sp_tbl <- read_csv("E://Documents/greenleaf_web_data/2019/Jan/Jan11_Year_end/analyst_target.csv")
...
Overall everybody is bullish. Targets range from 6% to 23% above the current price of 2600 on the S&P 500.

```{r target-chart,message=FALSE, warning=FALSE, echo=FALSE}

spx_price <- read_csv("E:/Documents/greenleaf_web_data/2019/Jan/Jan11_Year_end/spx_price.csv")

spx_price %>%
  filter(date >= "2018-10-01") %>%
  ggplot(aes(x = date, y = adjusted)) +
  geom_line(color = "brown", size = 1) +
  annotate("text", x = Sys.Date(), y = 2675, label = "Price on\nJan 11") +
